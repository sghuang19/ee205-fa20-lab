...
From the Intermediate Problem d we have already found the unit impulse response, so for finding the unit step response we can just convolve the unit step signal with the unit impulse response $h_1[n]$ and $h_2[n]$, which is $x_1[n]*h_1[n]$ and $x_1[n]*h_2[n]$.
...
To find out the results, all we need to do is to change the order of the convolution in Intermediate Problem e, which is now $h_1[n]*x_1[n]$ and $h_2[n]*x_1[n]$.
...
![Figure 2.5_f](https://raw.githubusercontent.com/SamuelHuang2019/SigSys-lab/master/figures/A2_2-10_e.png)
