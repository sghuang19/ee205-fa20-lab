...
Since for all three systems the input $x_3[n] = x_1 [n]+ 2*x_2[n]$, and the output $y_4[n] = y_1[n] + 2*y_2[n]$, so to analyse whether they are linear, we only need to check the plots for $y_3[n]$ and $y_4[n]$. 
...
To figure out whether they are time invariant, we need to compare $y_1[n]$ and $y_2[n]$, since the input $x_1[n]$ and $x_2[n]$ have the relation $x_1[n] = x_2[n-n_0]$, so if $y_1[n] = y_2[n-n_0]$, then the system is time invariant.
