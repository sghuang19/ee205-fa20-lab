...
The plots for them are shown below, and we can easily figure out that $x_1[n] * h_1[n]$ equals to $x_1[n] * h_1[n]$.
=======
We first find convolved x1 with h1, which is $x_1[n] * h1[n]$, then we change $x_1[n]$ into the impulse response and $h_1[n]$ into the input, and make convolution again, which is $x_1[n] * h_1[n]$.

The plots for them are shown below, and we can easily figure out that  $x_1[n] * h_1[n]$ equals to &h_1[n] * x_1[n]$.
>>>>>>> 3191d29d612456df09986db54f1428b5a639993a
...
We first convolved $x_1[n]*h_1[n]$ and $x__1[n]*h_2[n]$ separately, and then add them together and find out the plot. After that, we try to add $h_1[n]$ and $h_2[n]$ first, and convolved $x_1[n]*(h_1[n]+h_2[n])$.
